...
- `result`, `full_result` and `utf8_result` are returned as `simdutf_result`,
  `simdutf_full_result` and `simdutf_utf8_result`, through the `to_c_result`,
  `to_c_full_result` and `to_c_utf8_result` converters;
...
forwarding call, such as `simdutf_utf8_result_utf16_length`.
